...
import { clearTextureCache } from "./createTexture";
...
export class DisposalManager {
  ...
  private clearDataStructures(): void {
    ...
    // Clear texture cache to prevent stale textures on remount
    clearTextureCache();
  }
  ...
  private cleanupAnimationSystems(): void {
    ...
    try {
      // CRITICAL: Kill ALL GSAP animations on post-process shader before disposal
      // This prevents animations from continuing after remount
      if (this.host.postProcessShader) {
        // Kill all tweens targeting the shader object
        gsap.killTweensOf(this.host.postProcessShader);
        
        // Kill by property names as GSAP tracks them
        gsap.killTweensOf(this.host.postProcessShader, "distortionIntensity");
        gsap.killTweensOf(this.host.postProcessShader, "vignetteOffset");
        gsap.killTweensOf(this.host.postProcessShader, "vignetteDarkness");
      }
    } catch (error) {
